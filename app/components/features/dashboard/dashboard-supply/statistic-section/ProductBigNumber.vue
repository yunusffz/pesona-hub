<template>
  ...
  >
    ...
    <div v-else>
      ...
      <div
        class="border-t border-[#E7EFEA] flex flex-col gap-4 pt-2 font-inter text-xs"
      >
        <div class="flex justify-between" v-if="commodities[1]">
          <div class="text-[#4A5565]">{{ commodities[1]?.commodity }}:</div>
          <div class="text-[#1E1E1E] font-medium">
            {{ commodities[1]?.total_products }} produk
          </div>
        </div>
        <div class="flex justify-between" v-if="commodities[5]">
          <div class="text-[#4A5565]">{{ commodities[5]?.commodity }}:</div>
          <div class="text-[#1E1E1E] font-medium">
            {{ commodities[5]?.total_products }} produk
          </div>
        </div>

        <button class="text-[#174C36] text-center">Lihat Detail Lainnya</button>
      </div>
    </div>
  </section>
</template>
<script setup lang="ts">
  ...
  import { useCommoditiesPriorityDetailed } from "~/queries";
